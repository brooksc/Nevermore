...
//
//   NEVERMORE_EMAIL=you@gmail.com NEVERMORE_APP_PASSWORD=xxxx \
//     swift run nevermore-probe [--full]

// Diagnostic: open the app's real Application Support DB and report what
// AppModel would see, without any network. `nevermore-probe --count-appdb <email>`
if CommandLine.arguments.contains("--count-appdb") {
    guard let email = CommandLine.arguments.last, email.contains("@") else {
        print("usage: nevermore-probe --count-appdb <email>")
        exit(2)
    }
...
guard let address = env["NEVERMORE_EMAIL"], let password = env["NEVERMORE_APP_PASSWORD"] else {
    print("Set NEVERMORE_EMAIL and NEVERMORE_APP_PASSWORD.")
    exit(2)
}
let forceFull = CommandLine.arguments.contains("--full")

let dbPath = NSString(string: "~/.config/nevermore-mac/\(address).sqlite").expandingTildeInPath
